      ...
      <button onClick={() => setOpen(!open)} className="w-full flex items-start gap-3 p-4 text-left">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap mb-1.5">
            ...
            {q.company && (
              <Link href={`/companies/${q.company_slug}`} onClick={e => e.stopPropagation()} className="text-[10px] text-orange-400 bg-orange-500/10 border border-orange-500/20 px-2 py-0.5 rounded-full hover:bg-orange-500/20 transition-colors">
                {q.company}
              </Link>
            )}
          </div>
...
export default function QuestionsPage() {
  ...
  useEffect(() => {
    fetch('/api/questions/all')
      .then(r => r.json())
      .then(d => { setQuestions(d.questions ?? []); setCompanies(d.companies ?? []); setLoading(false) })
  ...
  }, [questions, filterTopic, filterLevel, filterCompany, search])

  const paginated = filtered.slice(page * PAGE, (page + 1) * PAGE)
  const totalPages = Math.ceil(filtered.length / PAGE)

  function resetFilters() {
    setFilterTopic('all'); setFilterLevel('all'); setFilterCompany('all'); setSearch(''); setPage(0)
  }
    ...
    <div className="min-h-screen bg-zinc-950 pt-20 pb-16">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-zinc-400 text-sm">{loading ? '…' : `${questions.length} questions`} · Browse, filter, and study at your own pace</p>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 mb-6 flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(filterTopic !== 'all' || filterLevel !== 'all' || filterCompany !== 'all' || search) && (
              <button onClick={resetFilters} className="text-xs text-zinc-500 hover:text-zinc-300 px-3 py-2 rounded-lg hover:bg-zinc-800 transition-colors">Clear filters</button>
            )}
            <span className="text-xs text-zinc-600 self-center ml-auto">{filtered.length} results</span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={resetFilters} className="text-sm text-orange-400 hover:text-orange-300">Clear all filters</button>
          </div>
        ) : (
          <>
            <div className="flex flex-col gap-2.5 mb-6">
              {paginated.map(q => <QuestionCard key={`${q.source}-${q.id}`} q={q} />)}
            </div>

            {/* Pagination */}
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3">
                <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="text-sm text-zinc-400 hover:text-zinc-100 disabled:opacity-30 bg-zinc-900 border border-zinc-800 px-4 py-2 rounded-xl transition-colors">← Prev</button>
                <span className="text-xs text-zinc-500">{page + 1} / {totalPages}</span>
                <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="text-sm text-zinc-400 hover:text-zinc-100 disabled:opacity-30 bg-zinc-900 border border-zinc-800 px-4 py-2 rounded-xl transition-colors">Next →</button>
              </div>
            )}
          </>
        )}
